...
if st.button("1. タイトル案を10個生成する"):
    if not keywords_str:
        ...
        # 記事の目的
        この記事の目的は、潜在顧客の共感を深く得て、最終的に自社製品への興味を自然な形で喚起することです。
...
if "title_list" in st.session_state and st.session_state["title_list"]:
    ...
    if st.button("2. 選択したタイトルで記事を生成する"):
        ...
        # もし概要が入力されていたら、その部分をプロンプトに「追加」する
        if summary:
            prompt_for_article += f"""
        # 参考概要
        ...
        - 結論と次のステップ: 記事のまとめとして、架空の自社製品『CloudFlow Dynamics』を自然な形で紹介し、読者が次のアクションを取りたくなるように締めくくる。
        - 読者がビジネスシーンを具体的にイメージできるように、「定例会議」「KPI管理」「リソース不足」のような、ビジネス用語を適切に含めてください。
        - 「承知いたしました」などの、他の余計な文章は一切含めないでください。
        """
